...
{
    float yaw_rate = 0.0f;//yaw速度
    float pitch_rate = 0.0f;//pitch速度

    if (rc_sensor.work_state == DEV_ONLINE)
    {
        // 遥控器模式
        if (Board_Rx_Info.state_pkt.car_state == 1)
        {
        ...
        // 键鼠模式
        else if (Board_Rx_Info.state_pkt.car_state == 2)
        {
...
static void gimbal_calc_output(gimbal_t *gimbal)
{
    ...
    gimbal->base_info.output_gimbal_p += gimbal->feedforward.pitch_torque_ff_nm;
    gimbal->base_info.output_gimbal_y += gimbal->feedforward.yaw_torque_ff_nm;
