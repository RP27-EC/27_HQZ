...
static void Motor_Set_Torque(rm_motor_t *motor)
{
	...
	motor->tx_info->tx_buff[Id+1] = (uint8_t)(motor->tx_info->torque_current_raw); /* 电流低字节 */
...
static void Motor_Set_Speed(rm_motor_t *motor)
{
	...
	my_speed_ctrl->measure = motor->rx_info->encoder_speed; /* 反馈速度 */
...
static void Motor_Set_Angle(rm_motor_t *motor)
{
	...
	my_speed_ctrl->target = my_angle_ctrl->out; /* 角度输出作速度目标 */
...
static void rm_motor_update(rm_motor_t *rm_motor, uint8_t *rxBuf)
{
    ...
    motor_info->encoder = CAN_01_GetMotorAngle(rxBuf); /* 编码器值 */
		Encoder_Sum_Cal(rm_motor);
		Encoder_to_Motor_Angle(rm_motor);
		motor_info->encoder_speed = CAN_23_GetMotorSpeed(rxBuf); /* 转速，rpm */
		motor_info->speed = RPM_to_Rads(rm_motor);
		motor_info->torque_current_raw = CAN_45_GetMotorCurrent(rxBuf); /* 原始电流 */
		Raw_Current_to_Torque(rm_motor);
    motor_info->temperature = CAN_6_GetMotorTemperature(rxBuf); /* 温度 */
    rm_motor->state->offline_cnt = 0; /* 收到反馈 */
}
...
{	
		int16_t torque_current = 0; /* 当前电机原始电流 */
		uint8_t Id; /* 报文中的字节偏移 */
		for(uint8_t i = 0; i < 4; i ++)
		{
			if(group->motor[i] != NULL)
			{
				Id = group->motor[i]->born_info->rxId*2; /* 电机槽位 */
				Torque_to_Raw_Current(group->motor[i]);
				torque_current = group->motor[i]->tx_info->torque_current_raw;
				group->tx_buff[Id] = (uint8_t)(torque_current >> 8); /* 高字节 */
				group->tx_buff[Id+1] = (uint8_t)(torque_current); /* 低字节 */
//				group->motor[i]->tx_info->torque = 0;
			}
		}
		
		CAN_SendData(group->hcan, group->stdId, group->tx_buff); /* 整组发送 */
		
		memset(group->tx_buff, 0, 8); /* 清空组帧缓存 */
}
...
void rm_group_init(rm_group_t *group)
{
		for(uint8_t i = 0; i < 4; i ++)
		{
			if(group->motor[i] != NULL)
			{
				group->motor[i]->single_init = rm_motor_init; /* 绑定初始化 */
				group->motor[i]->single_init(group->motor[i]);
			}
		}
	
	  group->group_set_torque = Group_Motor_Set_Torque; /* 绑定组发送 */
...
/* 编码器角度换算 */
/* 编码器值转换到输出轴角度，并按型号除减速比 */
static void Encoder_to_Motor_Angle(rm_motor_t *motor)
{
	if(motor->born_info->type == _3508_Reduction)
	motor->rx_info->motor_angle = ((float)motor->rx_info->encoder / 8191.f) * (float)PI * 2.f / _3508_REDUCT_RATIO; /* 3508 输出轴角 */
	else if(motor->born_info->type == _2006_Single)
	motor->rx_info->motor_angle = ((float)motor->rx_info->encoder / 8191.f) * (float)PI * 2.f / _2006_REDUCT_RATIO; /* 2006 输出轴角 */
	else
